...
## Если в другом боте пользователи не могут отправлять стикеры

Это почти всегда не проблема `sendSticker`, а **ограничения прав в группе**:

- В супергруппах за стикеры обычно отвечает право `can_send_other_messages`.
- Если оно выключено, обычные участники не смогут слать стикеры/гифки, даже если бот работает.
- Также могут быть персональные ограничения (mute/restrict) у конкретного пользователя.

Что проверить:

1. В настройках группы откройте права участников и включите отправку стикеров.
2. Проверьте, что у проблемных пользователей нет индивидуального ограничения.
3. Если используете код на Bot API, вызовите `getChat` и посмотрите `permissions.can_send_other_messages`.
4. Если бот должен управлять ограничениями, ему нужны права администратора с управлением участниками.

В этом проекте при старте добавлена автоматическая проверка `getChat`, и бот подскажет,
если у группы, вероятно, отключена отправка стикеров для участников.
